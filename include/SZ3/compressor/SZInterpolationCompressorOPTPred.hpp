...
// writefile("decompressed.dat", data, num_elements);
 ...
 private:
      ...
      size_t idx, size_t offset1, size_t offset2)
  {
    ...
    if ((quant_F> radius  && quant_H > radius)||
        (quant_F < radius  && quant_H < radius)) {
      // quant_pred_time+=pred_timer.stop();
  ...
  double block_interpolation_1d2(
  ...
  size_t quant_index_cmp= 0; // for compress
  double max_error;
  Quantizer quantizer;
  Encoder encoder;
  Lossless lossless;
