...
private:
  ...
  bool pattern_match2d(std::vector<int> &cpmap, int idx, int idy, int &xpadding,
    ...
    if (xpadding != 0 && ypadding != 0) {
      // T center_diff = 0 ;
      // center_diff = std::max(center_diff, std::abs(*get_value(idx,idy) - *get_value(idx+xpadding,idy)));
      // center_diff = std::max(center_diff, std::abs(*get_value(idx,idy) - *get_value(idx-xpadding,idy)));
      // center_diff = std::max(center_diff, std::abs(*get_value(idx,idy) - *get_value(idx,idy+ypadding)));
      // center_diff = std::max(center_diff, std::abs(*get_value(idx,idy) - *get_value(idx,idy-ypadding))); 
  ...
  bool pattern_match3d_interp(std::vector<int> &cpmap, int idx, int idy,
    ...
    if ((xpadding != 0 && ypadding != 0 && zpadding != 0)) {
      ...
      interp_error = std::sqrt(interp_error / (2.0 * (xpadding + ypadding + zpadding)+1));
